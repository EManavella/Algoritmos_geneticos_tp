...
def exportar_excel(todos_resultados, tiempos):
    ...
    for n_ciclos, historial in todos_resultados.items():
        ...
        for i, stats in enumerate(historial):
            ...
            valores_fila = [i, stats["maximo"], stats["minimo"], stats["promedio"], stats["desv_std"]]
            for col, valor in enumerate(valores_fila, start=1):
                cell = ws.cell(row=fila, column=col, value=int(valor) if col == 1 else round(valor, 10))
                estilo_dato(cell, fila_par=fila_par, negrita=(i == 0), color_fondo=color_fondo)
                if col == 1:
                    cell.number_format = "0"
 
        # fila verde con mejor solución
        fila_mejor = len(historial) + 3
        mejor      = max(historial, key=lambda s: s["maximo"])
        ws.merge_cells(f"A{fila_mejor}:E{fila_mejor}")
...
for n in GENERACIONES:
    print(f"\n{'#'*64}")
    print(f"  AG con {n} generaciones — Método: {METODO.capitalize()}")
    print(f"{'#'*64}")
