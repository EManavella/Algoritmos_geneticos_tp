...
def elegir_metodo():
    print("Método de selección:")
    print("1 - Ruleta")
    print("2 - Torneo")
    print("3 - Elitismo + Ruleta")
    opciones = {"1": "ruleta", "2": "torneo", "3": "elitismo"}
    while True:
        opcion = input("Ingresá el número (1/2/3): ").strip()
...
def estadisticas(poblacion, lista_fitness):
    ...
    return {
        ...
        "fitness_max": lista_fitness[idx_max],  # fitness normalizado del mejor individuo de la generación
    }
...
def calcular_estabilidad(historial):
    # σ calculada sobre los fitness máximos de cada generación
    # σ = sqrt( Σ(f_max_i - f̄_max)² / N ) donde f̄_max es el promedio de los fitness máximos
    fitnesses_max = [h["fitness_max"] for h in historial]
    n   = len(fitnesses_max)
    avg = sum(fitnesses_max) / n
    var = sum((f - avg) ** 2 for f in fitnesses_max) / n
    return var ** 0.5
...
def exportar_excel(todos_resultados, tiempos, metodo):
    ...
    # tabla principal: cols A-G (7 columnas, la 6 es cromosoma, la 7 es fitness max)
    # col H vacía como separador
    # resumen: arranca en col I (9)
    COL_RESUMEN = 9  # I

    encabezados = ["Generacion", "Maximo", "Minimo", "Promedio", "Desv. Std", "Cromosoma del maximo", "Fitness del maximo"]
    ...
    for n_ciclos, historial in todos_resultados.items():
        ...
        for i, stats in enumerate(historial):
            ws.append([
                ...
                float(f"{stats['desv_std']:.14f}"),  
                str(stats["cromosoma"]),
                round(stats["fitness_max"], 10),
            ])

        # calcula valores para el resumen
        val_obj_maximos  = [h["maximo"]      for h in historial]
        fitness_maximos  = [h["fitness_max"] for h in historial]
        mejor_fobj       = max(val_obj_maximos)
        gen_mejor_fobj   = val_obj_maximos.index(mejor_fobj)
        mejor_fitness    = max(fitness_maximos)
        gen_mejor_fit    = fitness_maximos.index(mejor_fitness)
        estabilidad      = calcular_estabilidad(historial)

        # escribe resumen al costado en col I, dejando H vacía como separador
        resumen = [
            ["RESUMEN",                    "",                           ""],
            ["",                           "Valor",                      "Generacion"],
            ["Funcion objetivo maxima",     round(mejor_fobj,   10),      gen_mejor_fobj],
            ["Fitness maximo",              round(mejor_fitness, 10),     gen_mejor_fit],
            ["Estabilidad",         float(f"{estabilidad:.14f}"), ""],
        ]
        for fila_idx, fila in enumerate(resumen, start=1):
            for col_offset, valor in enumerate(fila):
                ws.cell(row=fila_idx, column=COL_RESUMEN + col_offset, value=valor)

        # anchos de columna para que todo entre bien
        anchos = {
            1: 12,   # Generacion
            2: 14,   # Maximo
            3: 14,   # Minimo
